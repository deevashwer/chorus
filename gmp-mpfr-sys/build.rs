...
fn build_gmp(env: &Environment, lib: &Path, header: &Path) {
    ...
    if env.target == Target::Android {
        // set compiler and flags to build for android (not sure if we actually need all of these)
        // assumes $TOOLCHAIN env var is set
        let toolchain = env::var("ANDROID_TOOLCHAIN").unwrap();
        let api = "34"; // midSdkVersion
