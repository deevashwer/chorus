// The whole implementation of sparse_merkle_tree and AVD is taken from https://github.com/nirvantyagi/versa with small modifications
...
use sparse_merkle_tree::{MerkleTreeParameters};
use std::hash::Hash;
use crate::crypto::avd::error::MerkleTreeAVDError;

use std::{collections::HashMap, fmt};

pub trait SingleStepAVD: Sized {
    type Digest: Clone + Eq + Hash + Default + Serialize + DeserializeOwned;
    type PublicParameters: Clone + Default + Send + Sync;
    type LookupProof;
    type UpdateProof: Clone + Default;

    fn setup<R: Rng>(rng: &mut R) -> Result<Self::PublicParameters, MerkleTreeAVDError>;

    fn new<R: Rng>(rng: &mut R, pp: &Self::PublicParameters) -> Result<Self, MerkleTreeAVDError>;

    fn digest(&self) -> Result<Self::Digest, MerkleTreeAVDError>;

    fn lookup(
        &mut self,
        key: &[u8; 32],
    ) -> Result<(Option<(u64, [u8; 32])>, Self::Digest, Self::LookupProof), MerkleTreeAVDError>;

    fn update(
        &mut self,
        key: &[u8; 32],
        value: &[u8; 32],
    ) -> Result<(Self::Digest, Self::UpdateProof), MerkleTreeAVDError>;
    ...
    ) -> Result<bool, MerkleTreeAVDError>;
...
pub trait MerkleTreeAVDParameters {
    ...
    fn is_valid(&self) -> Result<bool, MerkleTreeAVDError> {
        Ok(Self::MerkleTreeParameters::is_valid()?)
    }
}
...
pub struct UpdateProof<P: MerkleTreeAVDParameters> {
    pub paths: Vec<MerkleTreePath<P::MerkleTreeParameters>>,
    pub indices: Vec<MerkleIndex>,
    pub keys: Vec<[u8; 32]>,
    pub versions: Vec<u64>,
    pub prev_values: Vec<[u8; 32]>,
    pub new_values: Vec<[u8; 32]>,
}
...
impl<P: MerkleTreeAVDParameters> SingleStepAVD for MerkleTreeAVD<P> {
    ...
    fn new<R: Rng>(_rng: &mut R, pp: &Self::PublicParameters) -> Result<Self, MerkleTreeAVDError> {
        ...
        let (probe, lookup_value) = match self.key_d.get(key) {
            ...
            None => {
                ...
                match (0..P::MAX_OPEN_ADDRESSING_PROBES).find(|i| {
                    ...
                    None => return Err(MerkleTreeAVDError::OpenAddressingOverflow(*key)),
                }
            }
        };
    ...
    fn batch_update(
        ...
        if kvs.len() > P::MAX_UPDATE_BATCH_SIZE as usize {
            return Err(MerkleTreeAVDError::UpdateBatchSize(kvs.len() as u64));
        }
        let update_proof = kvs
            .iter()
            .map(|(k, v)| self.update(k, v))
            .collect::<Result<Vec<(Self::Digest, Self::UpdateProof)>, MerkleTreeAVDError>>()?
            .iter()
            .fold(
                UpdateProof {
                    paths: Vec::new(),
    ...
    fn verify_lookup(
        ...
        {
            return Err(MerkleTreeAVDError::ProofFormat);
        }
        ...
        let skipped_probes_valid = (0..proof.paths.len() - 1)
            ...
            .collect::<Result<Vec<bool>, MerkleTreeAVDError>>()?
            .iter()
            .all(|b| *b);
...
mod tests {
    ...
    #[test]
    fn stress_test_batch_updates() {
        let mut rng = StdRng::seed_from_u64(0_u64);
        let start = Instant::now();
        let crh_parameters = H::setup(&mut rng).unwrap();
        let mut avd = TestMerkleTreeAVD::new(&mut rng, &crh_parameters).unwrap();
        let tree_size = 10usize.pow(6);
        // let mut kvs = vec![];
        ...
        for i in 0..tree_size {
            ...
            avd.update(&rng.gen::<[u8; 32]>(), &rng.gen::<[u8; 32]>()).unwrap();
        }
